...
def basic_preprocess(root_dir, total_swans_num, description_df):
    ...
    for label, folder_name in enumerate(folder_names):
        ...
        for image_file, mask_file in tqdm(zip(image_files, mask_files)):
            ...
            if(image.size[0] != mask.size[0] and image.size[1] != mask.size[1]): #skipping mismatching data
                continue
            if image.mode == "RGBA":
                image = image.convert("RGB")
            try:
                image.save(os.path.join(images_path, str(total_swans_num[0])+'.png'))
                mask.save(os.path.join(masks_path, str(total_swans_num[0])+'.png'))
                total_swans_num[0] +=1
                new_row = {"swan_id": label, "image_name": str(total_swans_num[0]) + ".png", "mask_name": str(total_swans_num[0]) + ".png"}
